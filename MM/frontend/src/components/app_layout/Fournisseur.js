...
import {getAllFournisseurs, getSelectedFournisseurs, createFournisseur, updateFournisseur, deleteFournisseur} from '../../actions/fournisseurAPI'
...
const columns = [
    ...
    { field: 'company_name', headerName: 'Fournisseur', width: 180 },
    { field: 'adress', headerName: 'Adress', width: 150 },
    { field: 'email', headerName: 'Email', width: 230 },
    { field: 'phone_number', headerName: 'Numero de telephone', width: 150 },
];
...
export default function Fournisseur(){
    ...
    const editFournisseurOpen = async () =>{
        ...
        if(selectionModel.length == 0){
          ...
          setRowData(await getSelectedFournisseurs(token, selectionModel[0])); 
        }
    ...
    const addFournSave = async () =>{
        ...
        if (test){
          ...
          const data = {
            company_name:fournisseurName,
            adress:fournisseurAdress,
            email:fournisseurEmailAdr,
            phone_number:fournisseurPhone,
          }
          ...
          setResponse(await createFournisseur(token, JSON.stringify(data))); 
    ...
    React.useEffect(() => {
        ...
        try{
          ...
          setFournisseurName(rowData.company_name);
          setFournisseurAdress(rowData.adress);
          setFournisseurEmailAdr(rowData.email)
          setFournisseurPhone(rowData.phone_number)
    ...
    React.useEffect(() => {
          ...
          const fetchData = async () => {
            try {
              const token = localStorage.getItem("auth_token");
              setData(await getAllFournisseurs(token));
              setLoading(false);
            } catch (error) {
...
<Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        {selectionError ? <Alt type='warning' message='Selectioner un fournisseur' onClose={()=> setSelectionError(false)} /> : null}
